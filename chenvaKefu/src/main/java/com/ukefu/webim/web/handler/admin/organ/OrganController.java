...
public class OrganController extends Handler{
    ...
    public ModelAndView index(ModelMap map , HttpServletRequest request , @Valid String organ) {
    	...
    	if(organList.size() > 0){
    		...
    		if(organData!=null){
    			map.addAttribute("userList", userRepository.findByOrgan(organData.getId()));
    		}
    	}
    ...
    public ModelAndView seluser(ModelMap map , HttpServletRequest request , @Valid String organ) {
    	map.addAttribute("userList", userRepository.findByOrgi(super.getOrgi(request))) ;
    	Organ organData = organRepository.findById(organ) ;
    	map.addAttribute("userOrganList", userRepository.findByOrgan(organ)) ;
